...
public class PinnedRoutes {
    ...
    public static void registerRoutes(Javalin app, AuthService authService, MessageService messageService,
        ...
        app.get("/api/pinned", ctx -> {
            ...
            try (Session s = DatabaseService.getInstance().getSessionFactory().openSession()) {
                ctx.json(ok(listMaps(s,
                    "SELECT c.* FROM chats c JOIN pinned_chats pc ON c.id = pc.chat_id WHERE pc.user_id = :uid ORDER BY pc.created_at DESC",
                    "uid", user.getId())));
            }
        });
        ...
        app.post("/api/pinned", ctx -> {
            ...
            try (Session s = DatabaseService.getInstance().getSessionFactory().openSession()) {
                s.beginTransaction();
                s.createNativeMutationQuery("INSERT INTO pinned_chats (user_id, chat_id, created_at) VALUES (:uid, :cid, NOW()) ON CONFLICT DO NOTHING")
                        .setParameter("uid", user.getId()).setParameter("cid", ((Number) chatId).longValue()).executeUpdate();
                s.getTransaction().commit();
            }
            ctx.json(ok(Map.of("message", "Chat pinned")));
        });
        ...
        app.delete("/api/pinned", ctx -> {
            ...
            try (Session s = DatabaseService.getInstance().getSessionFactory().openSession()) {
                s.beginTransaction();
                s.createNativeMutationQuery("DELETE FROM pinned_chats WHERE user_id = :uid AND chat_id = :cid")
                        .setParameter("uid", user.getId()).setParameter("cid", ((Number) chatId).longValue()).executeUpdate();
                s.getTransaction().commit();
            }
            ctx.json(ok(Map.of("message", "Chat unpinned")));
        });
    }
}
