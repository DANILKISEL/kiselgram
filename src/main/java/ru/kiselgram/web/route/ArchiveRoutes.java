        ...
        app.get("/api/archived", ctx -> {
            ...
            try (Session s = DatabaseService.getInstance().getSessionFactory().openSession()) {
                ctx.json(ok(listMaps(s,
                    "SELECT ac.*, c.title, c.type FROM archived_chats ac JOIN chats c ON c.id = ac.chat_id WHERE ac.user_id = :uid ORDER BY ac.created_at DESC",
                    "uid", user.getId())));
            }
        });

        app.post("/api/archived", ctx -> {
            User user = authService.getCurrentUser(ctx);
            if (user == null) { ctx.status(401).json(err("UNAUTHORIZED", "Not authenticated")); return; }
            ...
            try (Session s = DatabaseService.getInstance().getSessionFactory().openSession()) {
                s.beginTransaction();
                s.createNativeMutationQuery("INSERT INTO archived_chats (user_id, chat_id, created_at) VALUES (:uid, :cid, NOW()) ON CONFLICT DO NOTHING")
                        .setParameter("uid", user.getId()).setParameter("cid", ((Number) chatId).longValue()).executeUpdate();
                s.getTransaction().commit();
            }
            ctx.json(ok(Map.of("message", "Chat archived")));
        });

        app.delete("/api/archived", ctx -> {
            User user = authService.getCurrentUser(ctx);
            if (user == null) { ctx.status(401).json(err("UNAUTHORIZED", "Not authenticated")); return; }
            Map<String, Object> body = ctx.bodyAsClass(Map.class);
            Object chatId = body.get("chat_id");
            ...
            try (Session s = DatabaseService.getInstance().getSessionFactory().openSession()) {
                s.beginTransaction();
                s.createNativeMutationQuery("DELETE FROM archived_chats WHERE user_id = :uid AND chat_id = :cid")
                        .setParameter("uid", user.getId()).setParameter("cid", ((Number) chatId).longValue()).executeUpdate();
                s.getTransaction().commit();
            }
            ctx.json(ok(Map.of("message", "Chat unarchived")));
        });
    }
}
