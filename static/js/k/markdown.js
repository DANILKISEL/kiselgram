...
const MD_TAGS = new Set(['b','i','u','s','strong','em','del','ins','mark','code','pre','a','br','p','ul','ol','li','blockquote','h1','h2','h3','h4','hr','sub','sup','small']);
...
K.markdown = {
  ...
  render(text) {
    ...
    html = html.replace(/<(\/?)([a-zA-Z][a-zA-Z0-9]*)\b([^>]*)>/g, (match, slash, tag, attrs) => {
      ...
      // Keep href on <a>, strip everything else
      if (lower === 'a' && !slash) {
